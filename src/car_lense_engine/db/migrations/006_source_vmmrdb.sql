-- Widen the listings.source CHECK constraint to include 'vmmrdb'.
--
-- VMMRdb is ingested directly into listings + images (one synthetic listing
...
-- last statement, so a crash at any point leaves recoverable data. This is
-- the documented improvement over migration 004; see TODO.md known follow-ups.
--
...
--
-- Compare with migration 004, which did:
--   DROP TABLE listings  -- data only in listings_new
--   ALTER TABLE listings_new RENAME TO listings  -- gap closed
-- A crash between those two statements would lose the canonical-name table.
